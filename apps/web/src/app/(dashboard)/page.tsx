...
export default function DashboardPage() {
  ...
  const fetchDashboardData = useCallback(async () => {
    try {
      ...
      const [instancesRes, contactsRes, campaignsRes, runningRes] =
        await Promise.allSettled([
          api.get<InstancesResponse>('/instances', {
            params: { limit: '100' },
          }),
    ...
    completedCampaigns.length > 0
      ? Math.round(
          completedCampaigns.reduce((sum, c) => {
            const rate =
              c.total_sent > 0 ? (c.total_delivered / c.total_sent) * 100 : 0;
            return sum + rate;
      ...
      const daySends = campaigns.reduce((sum, c) => {
        ...
        if (date >= started && date <= finished) {
          const campaignDays = Math.max(
            1,
            Math.ceil((finished.getTime() - started.getTime()) / 86400000),
          );
