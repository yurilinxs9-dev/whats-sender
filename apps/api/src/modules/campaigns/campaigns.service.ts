...
export class CampaignsService {
  ...
  async findOne(id: string, tenantId: string) {
    const campaign = await this.prisma.campaign.findFirst({
      ...
      include: {
        template: {
          select: { id: true, nome: true, type: true, content: true },
        },
        contact_list: {
          select: {
            id: true,
  ...
  async create(data: CreateCampaignDto, tenantId: string) {
    ...
    if (!data.template_id && !data.inline_message) {
      throw new BadRequestException(
        'Informe um template ou uma mensagem direta',
      );
    }
    ...
    if (!contactList)
      throw new BadRequestException(
        'Lista de contatos nao encontrada ou nao pertence ao tenant',
      );
    ...
    if (instances.length !== data.instance_ids.length) {
      throw new BadRequestException(
        'Uma ou mais instancias nao foram encontradas ou nao pertencem ao tenant',
      );
    }
  ...
  async remove(id: string, tenantId: string) {
    ...
    if (!deletableStatuses.includes(campaign.status)) {
      throw new BadRequestException(
        'Somente campanhas em rascunho, canceladas ou concluidas podem ser excluidas',
      );
    }
  ...
  async cancel(id: string, tenantId: string) {
    ...
    if (!cancellableStatuses.includes(campaign.status)) {
      throw new BadRequestException(
        'Esta campanha nao pode ser cancelada no status atual',
      );
    }
